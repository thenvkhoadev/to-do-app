...
class TaskEditMobileNavBar extends ConsumerWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final name = (profile?.fullName?.trim().isNotEmpty == true
            ...
            : profile?.username?.trim().isNotEmpty == true
                ? profile!.username
                : metadata?['full_name'] ?? metadata?['username'] ?? user?.email ?? '?')
        .toString();
              ...
              children: [
                Row(
                  children: [
                    IconButton(
                      onPressed: onBack,
                      icon: const Icon(Icons.arrow_back_rounded, color: DashboardColors.onSurface),
