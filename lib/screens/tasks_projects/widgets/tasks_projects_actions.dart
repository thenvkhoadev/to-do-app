...
class TasksProjectsQuickActionDock extends StatelessWidget {
  const TasksProjectsQuickActionDock({this.compact = false, super.key});
    ...
    return TasksProjectsGlass(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: tasksProjectQuickActions.map((action) => Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: TasksProjectsQuickActionButton(action: action, compact: compact))).toList(),
        ),
      ),
    );
  }
...
class TasksProjectsQuickActionButton extends StatefulWidget {
  const TasksProjectsQuickActionButton({required this.action, this.compact = false, super.key});
